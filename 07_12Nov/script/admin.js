    ...
    data[0].breakfast.forEach((e) => {
        breakfastMeals.innerHTML += `
            <tr>
                ...
                <td>$${e.mealPrice}</td>
                <td>${e.mealStock}</td>
                <td style="display: none;">${e.mealId}</td>
                <td><button class="btn-x delete" id="${e.mealId}">Remove</button></td>
            </tr>
        `;
    });
    ...
    data[0].starter.forEach((e) => {
        starterMeals.innerHTML += `
            <tr>
                ...
                <td>$${e.mealPrice}</td>
                <td>${e.mealStock}</td>
                <td><button class="btn-x delete">Remove</button></td>
            </tr>
        `;
    });
    //add events funtion for all remove btns
...
function addEventsForRemoveBtns() {
    ...
    for (let i = 0; i < deleteMealBtns.length; i++) {
        ...
        deleteBtn.addEventListener('click', deleteMealFromDOM);
    }
}
//remove meal from DOM function
function deleteMealFromDOM(event) {
...
//!sa incerc sa fac fetch la inceput direct pe categorii
//remove meal from API - pentru 1 meal din beakfast(test)
//imi sterge din API dar nu mi le mai afiseaza pe restul
// function deleteMealFromAPI() {
//     fetch('https://myfirstproject-b0fb4.firebaseio.com/meals/0/breakfast/0.json', {
//         method: 'DELETE'
//     })
//         .then(function (res) {
//             return res.json();
